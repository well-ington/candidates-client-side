...
export const FilterButton: React.FC<TfilterButton> = ({
  filter,
  setFilter,
}) => {
  ...
  return (
    <div data-testid="filter-button-test" className={styles.container}>
          ...
          return filter[`${key}`].map((tech: string, techIndex: number) => {
            return (
              <Button
                ...
                action={() => alterArrayFilter(key, techIndex)}
              >
                {tech}
              </Button>
            );
